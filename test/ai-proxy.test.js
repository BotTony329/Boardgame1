...
test('clampPolicyResult 钳制越界数值并补齐缺省（逐回合量契约）', () => {
  const out = clampPolicyResult({
    verdict: '超正',
    narrative: 'x'.repeat(500),
    perTurn: { pop: 999, appeal: 55, stability: -100, food: 120, minerals: -999, energy: 88 },
    risks: ['a', 'b', 'c', 'd'],
  });
  assert.equal(out.verdict, 'neutral', '非法 verdict 回落 neutral');
  assert.equal(out.narrative.length, 300);
  assert.equal(out.perTurn.pop, CLAMPS.perTurnPop[1]);
  assert.equal(out.perTurn.appeal, CLAMPS.perTurnAppeal[1]);
  assert.equal(out.perTurn.stability, CLAMPS.perTurnStability[0]);
  assert.equal(out.perTurn.food, CLAMPS.perTurnFood[1]);
  assert.equal(out.perTurn.minerals, CLAMPS.perTurnMinerals[0]);
  assert.equal(out.perTurn.energy, CLAMPS.perTurnEnergy[1], '越界能源也被钳制');
...
test('clampPolicyResult 对完全畸形输入安全', () => {
  ...
  assert.equal(out.perTurn.pop, 0);
  assert.equal(out.perTurn.food, 0);
  assert.equal(out.source, 'qwen');
...
test('evaluatePolicy 正常路径：返回钳制后的裁决', async () => {
  const fetchImpl = mockFetch(['{"verdict":"positive","narrative":"万民称便","perTurn":{"pop":2,"appeal":4,"stability":3,"food":25,"minerals":-2,"energy":0},"risks":[]}']);
  const result = await evaluatePolicy(CONFIG, STATE, POLICY, fetchImpl);
  assert.equal(result.verdict, 'positive');
  assert.equal(result.perTurn.food, 25, 'AI 定夺的逐回合粮效直通');
  assert.equal(result.perTurn.appeal, 4);
...
test('evaluatePolicy 非法 JSON 自动重试一次后成功', async () => {
  const fetchImpl = mockFetch([
    '抱歉，我无法输出 JSON。',
    '{"verdict":"negative","narrative":"民怨沸腾","perTurn":{"pop":-3,"appeal":-4,"stability":-5,"food":-10,"minerals":0,"energy":0},"risks":["动荡"]}',
  ]);
  const result = await evaluatePolicy(CONFIG, STATE, POLICY, fetchImpl);
