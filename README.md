...
**军团与战争（回合制战棋）**：征募的后备兵员可**组建军团**，落到地图上行军（每回合 1 格）。选中军团后点击相邻格即上下文行动：己格调防、无主地镇压拓疆、敌格开战；也可**修筑工事**（两级防御加成）、**坚守**（防御 +35%）或以粮**和平拓疆**。战斗消耗能源；攻占敌土即宣战。

**传檄归降**：列国关系面板可「劝降」——国力（含士气折算）达对方 2 倍且关系友善（≥40）时，可兵不血刃并其国；条件不足而强劝，则触怒对方，邦交钉在敌视、即刻开战。
...
public/
  ...
  js/
    ...
    engine/             浏览器与 Node 测试共用的引擎（无 DOM 依赖）
      ...
      armies.js         军团系统：行军/攻伐/工事/坚守/殖民拓疆 + AI 军团
